...
// Only serve static if the dist folder exists; otherwise log a warning
const fs = require('fs');
if (fs.existsSync(clientDist)) {
  app.use(express.static(clientDist));
  // SPA fallback - ensure API routes (starting with /api) are not overridden
  app.get('*', (req, res) => {
    if (req.path.startsWith('/api/')) return res.status(404).json({ error: 'Not found' });
    res.sendFile(path.join(clientDist, 'index.html'));
  });
} else {
  console.warn('Warning: frontend build not found at', clientDist, '- static files will NOT be served.');
  // Optionally provide root path message so visiting root isn't empty
  app.get('/', (req, res) => res.send('API running. Frontend not built (no ../dist).'));
}
